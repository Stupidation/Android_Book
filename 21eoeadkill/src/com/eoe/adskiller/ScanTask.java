...
public class ScanTask extends Thread {
	...
	public void run() {
		...
		// Cursor cursor = db.rawQuery("select * from ads where adId=?",new
		// String[]{""});
		Cursor cursor = db.query("ads", new String[] { "receivername" }, null, null, null, null, null, null);
		...
		for (int i = 0; i < packages.size(); i++) {
			...
			if ((packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0) {
				...
				try {
					...
					am = mycontext.createPackageContext(packageInfo.packageName, 0).getAssets();
