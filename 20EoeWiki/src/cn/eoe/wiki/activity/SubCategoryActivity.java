	...
	protected void generateCategories(CategoryJson responseObject) {
		...
		if (categories != null) {
			for (CategoryChild category : categories) {
				...
				tv.setOnClickListener(new SubCategoryListener(category.getName(), mParentCategory.getName(), "", category.getUri(),
			...
			// LayoutParams noCategoryParams = new
	...
	private void buildCategories(ViewGroup parent, List<CategoryChild> categories, int floot) {
		if (categories != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				tvChild.setPadding(WikiUtil.dip2px(mContext, 20) * floot, WikiUtil.dip2px(mContext, 5), WikiUtil.dip2px(mContext, 5),
						WikiUtil.dip2px(mContext, 5));
