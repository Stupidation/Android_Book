...
public class GeneralDao<T extends DatabaseColumn> {
	...
	public Cursor queryByPage(int page, int length) {
		...
		return SqliteWrapper.query(context, context.getContentResolver(), uri, this.tableClass.getColumns(), null, null, DatabaseColumn._ID
				+ " limit " + (page - 1) * length + "," + length);
	}

	public Cursor queryById(long id) {
		return SqliteWrapper.query(context, context.getContentResolver(), Uri.withAppendedPath(uri, String.valueOf(id)),
				this.tableClass.getColumns(), null, null, null);
	}

	public Cursor queryByParameter(String name, long value) {
		return SqliteWrapper
				.query(context, context.getContentResolver(), uri, this.tableClass.getColumns(), name + "=" + value, null, null);
	}

	public Cursor queryByParameter(String name, String value) {
		return SqliteWrapper.query(context, context.getContentResolver(), uri, this.tableClass.getColumns(), name + "='" + value + "'",
				null, null);
	}
