...
public class FSExplorerActivity extends Activity implements OnItemClickListener {
	...
	private void refreshListItems(String path) {
		...
		SimpleAdapter notes = new SimpleAdapter(this, list, R.layout.file_row, new String[] { "name", "path", "img" }, new int[] {
				R.id.name, R.id.desc, R.id.img });
	...
	private void goToParent() {
		...
		if (str_pa == null) {
			Toast.makeText(FSExplorerActivity.this, getString(R.string.is_root_dir), Toast.LENGTH_SHORT).show();
			refreshListItems(path);
		} else {
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
		...
		if (position == 0) {
			...
			if (file.isDirectory())
				...
				Toast.makeText(FSExplorerActivity.this, getString(R.string.is_file), Toast.LENGTH_SHORT).show();
		}
